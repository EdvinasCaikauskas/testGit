...
const StartGame = ({choosePlayer}) => {


    return (
        <div>
            <div className="players-container">
                {players.map((x, i) => <div className="players" onClick={() => choosePlayer(x)}>
                    <img src={x.image} alt=""/>
                    <h1>Race: {x.race}</h1>
                    <h3>Damage: {x.damage}</h3>
                    <h3>Health: {x.health}</h3>
                    <h3>Energy: {x.energy}</h3>
                    <h3>Stamina: {x.stamina}</h3>
                    <h3>Strength: {x.strength}</h3>
                    <h3>Slots: {x.inventorySlots}</h3>
                    <h3>Gold: {x.gold}</h3>
                </div>)}
            </div>
        </div>
